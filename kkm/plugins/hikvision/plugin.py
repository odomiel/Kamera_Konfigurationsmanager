...
"""Hikvision plugin — adapts the ISAPI/SADP modules to ``VendorPlugin``.

Like the Axis plugin this is a thin wrapper: the heavy lifting lives in
:mod:`kkm.plugins.hikvision.isapi` (ISAPI/Digest, stdlib-only) and
:mod:`kkm.plugins.hikvision.discovery` (SADP). ONVIF users are handled through the
**shared** ONVIF SOAP client (:mod:`kkm.plugins.onvif.soap`) rather than a second
copy — a Hikvision device speaks standard ONVIF for that part.

**Experimental** (``experimental = True``): the discovery/ISAPI paths follow the
documented protocol but have not been verified against real hardware, so the plugin
is flagged in the plugin manager and shipped **off by default**. Capabilities
mirror the feasibility note (`HIKVISION_PLUGIN_RECHERCHE.md`): no ``FIRMWARE_CHECK``
(no open firmware repo like Axis') and no ``CONFIG`` (``configurationData`` is an
opaque per-device blob, not a fleet template).
"""
...
class HikvisionPlugin(VendorPlugin):
    id = "hikvision"
    name = "Hikvision"
    experimental = True
    ...
    def _onvif_auth(self, camera: dict, creds: Credentials):
        """URL + Zeitversatz fuer die ONVIF-Benutzerverwaltung (wie im ONVIF-Plugin:
    ...
    def check_online(self, camera: dict, creds: Credentials | None = None) -> bool:
        ...
        try:
            return isapi.is_online(ip, scheme=creds.scheme, port=creds.port,
    ...
    def device_info(self, camera: dict, creds: Credentials) -> dict:
        ip = self.ip_of(camera)
        return isapi.get_device_info(ip, creds.username, creds.password,
                                     **self._conn(creds))
    ...
    def is_unconfigured(self, camera: dict, creds: Credentials | None = None) -> bool:
        ...
        creds = creds or Credentials()
        try:
            return isapi.is_unconfigured(ip, scheme=creds.scheme, port=creds.port,
                                         timeout=min(creds.timeout, 5))
    ...
    def upgrade_firmware(self, camera, creds: Credentials, firmware_path,
        ...
        ip = self.ip_of(camera)
        return isapi.upgrade_firmware(ip, creds.username, creds.password,
